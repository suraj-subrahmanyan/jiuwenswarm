...
revised target Capsule template (draft 2, updated after the adversarial audit);
it is a proposal, not current runtime behavior. The adversarial findings and
...
status model with precise axis definitions (LL-3).
